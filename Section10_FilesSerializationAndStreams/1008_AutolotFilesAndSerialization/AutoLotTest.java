...
public class AutoLotTest extends junit.framework.TestCase {
    ...
    {
        Vehicle v2 = new Car();
        v2.setVIN(Common.VIN);
        al.AddVehicle(v2);
        Vehicle v = al.FindVehicle(Common.VIN);
        assertNotNull("The vehicle was not found as expected"
                        , v);
        //should not find invalid vehicle
        Vehicle nv = al.FindVehicle("213523523523");
        assertNull("A vehicle that does not exist should not be found"
                        , nv);
    }
}