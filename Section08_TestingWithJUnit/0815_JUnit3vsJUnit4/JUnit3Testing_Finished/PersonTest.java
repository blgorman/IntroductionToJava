//for J-Unit3, we don't import the libraries
/*
...
/**
 * Test the Person Class using JUnit v3
 * @author Brian
 *
 */
//note: in JUnit3 we extend the test framework
public class PersonTest extends junit.framework.TestCase
{
	...
	//junit 3 does not allow for any attributes
	//@BeforeClass
	...
	//to make a method a test in JUnit 3
	//we preface all of our methods with "test":
	//*** Luckily, we have already done this :)
	//@Test
	...
	/*
	 * Obviously, we can no longer expect a thrown exception
	 * but, we still need to test for them, right?
	 * So what do we do?????
	 * 
	 * Use a boolean set to false
	 * Use a try..catch with the specific exception we are testing
	 * and set the boolean to true if we catch the exception
	 * then just assertTrue on the boolean:
	 * 
	 *  boolean caught = false;
	 *  try
	 *  {
	 *     //perform your code that should throw ex
	 *  }
	 *  catch (specific ex)
	 *  {
	 *  	caught = true;
	 *  }
	 *  assertTrue("Exception x was not thrown as expected when ..."
	 *  				, caught);
	 *  
	 *  Now we have another advantage -- test execution continues
	 *  even when exception happens.  We should still be careful
	 *  not to write too many tests in one method, however.
	 *  
	 *  One more advantage is we can also have stacked exceptions
	 *  in our testing (i.e. IllegalArgument and Arithmetic can 
	 *  both be tested in same block).
	 */
	...
	public void testSetInvalidFirstName()
	{
		...
		assertTrue("Illegal Argument Exception "
				+ "should be thrown when attempting to set"
				+ " invalid first name", caught);
	}
